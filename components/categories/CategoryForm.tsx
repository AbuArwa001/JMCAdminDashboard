...
export default function CategoryForm({ initialData, isEditing = false }: CategoryFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && initialData?.id) {
                await api.patch(`/api/v1/categories/${initialData.id}/`, formData);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 max-w-md bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div>
